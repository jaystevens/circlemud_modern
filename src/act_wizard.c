            ...
            switch (ZOCMD.command) {
                ...
                case 'T':
                    send_to_char(ch, "%sAttach trigger %s%s%s [%s%d%s] to %s\r\n", ZOCMD.if_flag ? " then " : "", cyn,
                                 ...
                                 ((ZOCMD.arg1 == MOB_TRIGGER) ? "mobile" : ((ZOCMD.arg1 == OBJ_TRIGGER) ? "object"
                                                                                                        : ((ZOCMD.arg1 ==
                                                                                                            WLD_TRIGGER)
                                                                                                           ? "room"
                                                                                                           : "????"))));
    ...
    switch (GET_OBJ_TYPE(j)) {
        ...
        case ITEM_STAFF:
            send_to_char(ch, "Spell: %s at level %d, %d (of %d) charges remaining\r\n", skill_name(GET_OBJ_VAL(j, 3)),
                         GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 2), GET_OBJ_VAL(j, 1));
            break;
        case ITEM_WEAPON:
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
    ...
    send_to_char(ch, "%s%s, Lev: [%s%2d%s], XP: [%s%7d%s], Align: [%4d]\r\n", IS_NPC(k) ? "Mobile" : "Class: ",
                 IS_NPC(k) ? "" : buf, CCYEL(ch, C_NRM), GET_LEVEL(k), CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), GET_EXP(k),
    ...
    send_to_char(ch, "Str: [%s%d/%d%s]  Int: [%s%d%s]  Wis: [%s%d%s]  "
                     "Dex: [%s%d%s]  Con: [%s%d%s]  Cha: [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_STR(k), GET_ADD(k),
                 CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_INT(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_WIS(k),
                 CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_DEX(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_CON(k),
                 CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_CHA(k), CCNRM(ch, C_NRM));
    ...
    if (listall) {
        ...
        tmp = snprintf(bufptr, left, "%3d %-30.30s%s By: %-10.10s%s Age: %3d; Reset: %3d (%s); Range: %5d-%5d\r\n",
                       ...
                       zone_table[zone].reset_mode ? ((zone_table[zone].reset_mode == 1)
                                                   ...
                                                   : "Never reset", zone_table[zone].bot, zone_table[zone].top);
        j = k = l = m = n = o = 0;
        ...
        tmp += snprintf(bufptr + tmp, left - tmp, "       Zone stats:\r\n"
                                                  "       ---------------\r\n"
                                                  "         Flags:    %s\r\n"
                                                  "         Min Lev:  %2d\r\n"
                                                  "         Max Lev:  %2d\r\n"
                                                  ...
                                                  "         Shops:    %2d\r\n"
                                                  "         Triggers: %2d\r\n"
                                                  "         Quests:   %2d\r\n", buf, zone_table[zone].min_level,
                        zone_table[zone].max_level, j, k, l, m, n, o);
    ...
    switch (l) {
        ...
        case 4:
            ...
            send_to_char(ch, "Current stats:\r\n"
                         ...
                         top_of_world + 1, top_of_zone_table + 1, top_of_trigt + 1, top_shop + 1, buf_largecount,
                         total_quests, buf_switches, buf_overflows, global_lists->iSize);
            break;
...
{
    const char *set_levels[] = {
        "Imm",
        "God",
        "GrGod",
        "IMP"};
    const char *set_targets[] = {
        "PC",
        "NPC",
        "BOTH"};
...
} zarmor[] = {{ITEM_WEAR_FINGER, 10, "Ring"},
    ...
    for (i = 0; i < top_of_mobt; i++) {
        if (real_zone_by_thing(mob_index[i].vnum) == zrnum) {  /*is mob in this zone?*/
            ...
            if ((AFF_FLAGGED(mob, AFF_CHARM) || AFF_FLAGGED(mob, AFF_POISON)) && (found = 1)) {
                len += snprintf(buf + len, sizeof(buf) - len, "- Has illegal affection bits set (%s %s)\r\n",
    ...
    for (i = 0; i < top_of_world; i++) {
        if (world[i].zone == zrnum) {
            ...
            if (ROOM_FLAGGED(i, ROOM_ATRIUM) || ROOM_FLAGGED(i, ROOM_HOUSE) || ROOM_FLAGGED(i, ROOM_HOUSE_CRASH) ||
                ...
                len += snprintf(buf + len, sizeof(buf) - len, "- Has illegal affection bits set (%s %s %s %s %s)\r\n",
...
ACMD(do_file)
{
    ...
    } fields[] = {{"xnames",    LVL_GOD, XNAME_FILE,         true},
    ...
    snprintf(buf, sizeof(buf), "\tgFile:\tn %s\tg; Min. Level to read:\tn %d\tg; File Location:\tn %s\tg\r\n"
                               "File size (bytes):\tn %ld\tg; Total num lines:\tn %d\r\n", fields[l].cmd,
             fields[l].level, fields[l].file, (long) req_file_size, req_file_lines);
