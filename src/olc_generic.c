...
static struct {
    ...
    const char *message;
} save_types[] = {{SL_MOB, save_mobiles, "mobile"},
    ...
    for (i = genolc_zone_bottom(zrnum); i <= zone_table[zrnum].top; i++) {
        if ((rshop = real_shop(i)) != NOWHERE) {
            ...
            fprintf(shop_file, "%s~\n"
                    ...
                    S_NOCASH1(shop) ? S_NOCASH1(shop) : "%s Ke?!", S_NOCASH2(shop) ? S_NOCASH2(shop) : "%s Ke?!",
                    S_BUY(shop) ? S_BUY(shop) : "%s Ke?! %d?", S_SELL(shop) ? S_SELL(shop) : "%s Ke?! %d?",
                    S_BROKE_TEMPER(shop), S_BITVECTOR(shop), mob_index[S_KEEPER(shop)].vnum % 100, S_NOTRADE(shop));
...
static int export_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
    ...
    fprintf(fd, "%d %d %d %d %d %d %d %d %d E\n"
                "%d %d %d %dd%d+%d %dd%d+%d\n", MOB_FLAGS(mob)[0], MOB_FLAGS(mob)[1], MOB_FLAGS(mob)[2],
            MOB_FLAGS(mob)[3], AFF_FLAGS(mob)[0], AFF_FLAGS(mob)[1], AFF_FLAGS(mob)[2], AFF_FLAGS(mob)[3],
            GET_ALIGNMENT(mob), GET_LEVEL(mob), 20 - GET_HITROLL(mob), GET_AC(mob) / 10, GET_HIT(mob), GET_MANA(mob),
            GET_MOVE(mob), GET_NDD(mob), GET_SDD(mob), GET_DAMROLL(mob));
    ...
    for (i = genolc_zone_bottom(zrnum); i <= zone_table[zrnum].top; i++) {
        ...
        if ((rnum = real_room(i)) != NOWHERE) {
            ...
            fprintf(room_file, "#QQ%02d\n"
                               "%s%c\n"
                               "%s%c\n"
                               "QQ %d %d %d %d %d\n", room->number % 100, room->name ? room->name : "Untitled",
                    STRING_TERMINATOR, buf, STRING_TERMINATOR, room->room_flags[0], room->room_flags[1],
    ...
    for (i = genolc_zone_bottom(zrnum); i <= zone_table[zrnum].top; i++) {
        ...
        if ((rnum = real_trigger(i)) != NOTHING) {
            ...
            fprintf(trig_file, "%s%c\n"
                    ...
                    STRING_TERMINATOR, trig->attach_type, *bitBuf ? bitBuf : "0", GET_TRIG_NARG(trig),
