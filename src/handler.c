...
void char_to_room(struct char_data *ch, room_rnum room)
{
    if (ch == NULL || room == NOWHERE || room > top_of_world)
        basic_mud_log("SYSERR: Illegal value(s) passed to char_to_room. (Room: %d/%d Ch: %p", room, top_of_world, (void *) ch);
    else {
...
void obj_to_char(struct obj_data *object, struct char_data *ch)
{
    if (object && ch) {
        ...
        basic_mud_log("SYSERR: NULL obj (%p) or char (%p) passed to obj_to_char.", (void *) object, (void *) ch);
}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
    ...
    if (GET_EQ(ch, pos)) {
        basic_mud_log("SYSERR: Char is already equipped: %s, %s", GET_NAME(ch), obj->short_description);
        return;
    }
    if (obj->carried_by) {
...
void obj_to_room(struct obj_data *object, room_rnum room)
{
    if (!object || room == NOWHERE || room > top_of_world)
        basic_mud_log("SYSERR: Illegal value(s) passed to obj_to_room. (Room #%d/%d, obj %p)", room, top_of_world,
            (void *) object);
    else {
...
void obj_from_room(struct obj_data *object)
{
    ...
    if (!object || IN_ROOM(object) == NOWHERE) {
        basic_mud_log("SYSERR: NULL object (%p) or obj not in a room (%d) passed to obj_from_room", (void *) object,
            IN_ROOM(object));
        return;
    }
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to)
{
    ...
    if (!obj || !obj_to || obj == obj_to) {
        basic_mud_log("SYSERR: NULL object (%p) or same source (%p) and target (%p) obj passed to obj_to_obj.", (void *) obj,
...
void extract_char_final(struct char_data *ch)
{
    ...
    if (IN_ROOM(ch) == NOWHERE) {
        basic_mud_log("SYSERR: NOWHERE extracting char %s. (%s, extract_char_final)", GET_NAME(ch), __FILE__);
        exit(1);
    }
    ...
    for (temp = character_list; temp; temp = temp->next) {
        ...
        if (!IS_NPC(ch) && GET_POS(ch) == POS_DEAD && MEMORY(temp)) {
            forget(temp, ch);
        } /* forget() is safe to use without a check. */
    }
