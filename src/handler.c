...
void char_to_room(struct char_data *ch, room_rnum room)
{
    if (ch == NULL || room == NOWHERE || room > top_of_world)
        log("SYSERR: Illegal value(s) passed to char_to_room. (Room: %d/%d Ch: %p", room, top_of_world, (void *) ch);
    else {
...
void obj_from_room(struct obj_data *object)
{
    ...
    if (!object || IN_ROOM(object) == NOWHERE) {
        log("SYSERR: NULL object (%p) or obj not in a room (%d) passed to obj_from_room", (void *) object,
            IN_ROOM(object));
        return;
    }
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to)
{
    ...
    if (!obj || !obj_to || obj == obj_to) {
        log("SYSERR: NULL object (%p) or same source (%p) and target (%p) obj passed to obj_to_obj.", (void *) obj,
