...
/* player_special_data_saved */
        ...
        if (psds->conditions[HUNGER] && player.level < LVL_IMMORT && psds->conditions[HUNGER] != PFDEF_HUNGER) {
            fprintf(outfile, "Hung: %d\n", (int) psds->conditions[0]);
        }
        if (psds->conditions[THIRST] && player.level < LVL_IMMORT && psds->conditions[THIRST] != PFDEF_THIRST) {
