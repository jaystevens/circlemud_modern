                    ...
                    case ITEM_LIGHT:
                        ...
                        if (v1 == -1) {
                            ...
                            tmp_len = snprintf(buf + len, sizeof(buf) - len,
                                               ...
                                               QYEL, ov, QCYN, QNRM, v1, QCYN, obj_proto[r_num].short_description,
                                               QNRM);
                        }
                        break;
    ...
    for (i = 0; i < MAX_OBJ_LIST; i++) {
        if ((r_num = real_object(lst[i].vobj)) != NOTHING) {
            tmp_len = snprintf(buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%8d%s] %s%3d %s%-*s %s[%s]%s%s\r\n", QGRN,
                               ++found, QNRM, QCYN, QYEL, lst[i].vobj, QCYN, QYEL, lst[i].val, QCYN,
                               42 + count_color_chars(obj_proto[num].short_description),
                               obj_proto[r_num].short_description, QYEL,
...
static void list_rooms(struct char_data *ch, zone_rnum rnum, room_vnum vmin, room_vnum vmax)
{
    ...
    len = strlcpy(buf, "Index VNum    Room Name                                    Exits\r\n"
                       "----- ------- -------------------------------------------- -----\r\n", sizeof(buf));
...
static void list_mobiles(struct char_data *ch, zone_rnum rnum, mob_vnum vmin, mob_vnum vmax)
{
    ...
    len = strlcpy(buf, "Index VNum    Mobile Name                                  Level\r\n"
                       "----- ------- -------------------------------------------- -----\r\n", sizeof(buf));
...
static void list_objects(struct char_data *ch, zone_rnum rnum, obj_vnum vmin, obj_vnum vmax)
{
    ...
    len = strlcpy(buf, "Index VNum    Object Name                                  Object Type\r\n"
                       "----- ------- -------------------------------------------- ----------------\r\n", sizeof(buf));
    ...
    for (i = 0; i <= top_of_objt; i++) {
        if (obj_index[i].vnum >= bottom && obj_index[i].vnum <= top) {
            ...
            len += snprintf(buf + len, sizeof(buf) - len, "%s%4d%s) [%s%-5d%s] %s%-*s %s[%s]%s%s\r\n", QGRN, counter,
                            QNRM, QGRN, obj_index[i].vnum, QNRM, QCYN,
                            count_color_chars(obj_proto[i].short_description) + 44, obj_proto[i].short_description,
                            QYEL, item_types[obj_proto[i].obj_flags.type_flag], QNRM,
...
static void list_shops(struct char_data *ch, zone_rnum rnum, shop_vnum vmin, shop_vnum vmax)
{
    ...
    send_to_char(ch, "Index VNum    RNum    Shop Room(s)\r\n"
                     "----- ------- ------- -----------------------------------------\r\n");
...
static void list_zones(struct char_data *ch, zone_rnum rnum, zone_vnum vmin, zone_vnum vmax, char *name)
{
    ...
    len = snprintf(buf, sizeof(buf), "VNum  Zone Name                      Builder(s)\r\n"
                                     "----- ------------------------------ --------------------------------------\r\n");
        ...
        if (zone_table[i].number >= bottom && zone_table[i].number <= top) {
            if ((!use_name) || (is_name(name, zone_table[i].builders))) {
                ...
                tmp_len = snprintf(buf + len, sizeof(buf) - len, "[%s%3d%s] %s%-*s %s%-1s%s\r\n", QGRN,
                                   zone_table[i].number, QNRM, QCYN, count_color_chars(zone_table[i].name) + 30,
                                   zone_table[i].name, QYEL, zone_table[i].builders ? zone_table[i].builders : "None.",
                                   QNRM);
                len += tmp_len;
                if (len > sizeof(buf)) {
                    break;
...
void print_zone(struct char_data *ch, zone_vnum vnum)
{
    ...
    send_to_char(ch, "%sVirtual Number = %s%d\r\n"
                     "%sName of zone   = %s%s\r\n"
                     "%sBuilders       = %s%s\r\n"
                     "%sLifespan       = %s%d\r\n"
                     "%sAge            = %s%d\r\n"
                     ...
                     "%s   Shops       = %s%d\r\n"
                     "%s   Triggers    = %s%d\r\n"
                     "%s   Quests      = %s%d%s\r\n", QGRN, QCYN, zone_table[rnum].number, QGRN, QCYN,
                 zone_table[rnum].name, QGRN, QCYN, zone_table[rnum].builders, QGRN, QCYN, zone_table[rnum].lifespan,
                 QGRN, QCYN, zone_table[rnum].age, QGRN, QCYN, zone_table[rnum].bot, QGRN, QCYN, zone_table[rnum].top,
                 QGRN, QCYN, zone_table[rnum].reset_mode ? ((zone_table[rnum].reset_mode == 1)
                                                            ? "Reset when no players are in zone." : "Normal reset.")
                                                         : "Never reset", QGRN, QCYN, buf, QGRN, QCYN,
                 zone_table[rnum].min_level, QGRN, QCYN, zone_table[rnum].max_level, QGRN, QGRN, QCYN, size_rooms, QGRN,
                 QCYN, size_objects, QGRN, QCYN, size_mobiles, QGRN, QCYN, size_shops, QGRN, QCYN, size_trigs, QGRN,
                 QCYN, size_quests, QNRM);
}
...
static void list_triggers(struct char_data *ch, zone_rnum rnum, trig_vnum vmin, trig_vnum vmax)
{
    ...
    send_to_char(ch, "Index VNum    Trigger Name                                  Type\r\n"
                     "----- ------- --------------------------------------------- ---------\r\n");
