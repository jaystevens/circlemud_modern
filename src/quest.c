...
static const char *quest_mort_usage = "Usage: quest list | history | progress | join <nn> | leave";

static const char *quest_imm_usage = "Usage: quest list | history | progress | join <nn> | leave | status <vnum>";
    ...
    if (--GET_QUEST_COUNTER(ch) <= 0) {
        ...
        if (QST_GOLD(rnum)) {
            if ((IS_HAPPYHOUR) && (IS_HAPPYGOLD)) {
                ...
                send_to_char(ch, "You have been awarded %d gold coins for your service.\r\n", happy_gold);
            } else {
        ...
        if (QST_EXP(rnum)) {
            ...
            if ((IS_HAPPYHOUR) && (IS_HAPPYEXP)) {
                ...
                send_to_char(ch, "You have been awarded %d experience for your service.\r\n", happy_exp);
            } else {
                send_to_char(ch, "You have been awarded %d experience points for your service.\r\n", QST_EXP(rnum));
            }
        }
        ...
        if ((real_quest(QST_NEXT(rnum)) != NOTHING) && (QST_NEXT(rnum) != vnum) && !is_complete(ch, QST_NEXT(rnum))) {
            ...
            set_quest(ch, rnum);
            send_to_char(ch, "The next stage of your quest awaits:\r\n%s", QST_INFO(rnum));
        }
    }
    save_char(ch);
}
    ...
    for (i = 0; i < GET_NUM_QUESTS(ch); i++) {
        if ((rnum = real_quest(ch->player_specials->saved.completed_quests[i])) != NOTHING) {
            send_to_char(ch, "\tg%4d\tn) \tc%-52.52s\tn \ty%s\tn\r\n", ++counter, QST_DESC(rnum),
                         (real_mobile(QST_MASTER(rnum)) == NOBODY) ? "Unknown" : GET_NAME(
...
static void quest_join(struct char_data *ch, struct char_data *qm, char argument[MAX_INPUT_LENGTH])
{
    ...
    if (!*argument) {
        snprintf(buf, sizeof(buf), "%s What quest did you wish to join?", GET_NAME(ch));
    } else if (GET_QUEST(ch) != NOTHING) {
        snprintf(buf, sizeof(buf), "%s But you are already part of a quest!", GET_NAME(ch));
    } else if ((vnum = find_quest_by_qmnum(ch, GET_MOB_VNUM(qm), atoi(argument))) == NOTHING) {
        snprintf(buf, sizeof(buf), "%s I don't know of such a quest!", GET_NAME(ch));
    } else if ((rnum = real_quest(vnum)) == NOTHING) {
        snprintf(buf, sizeof(buf), "%s I don't know of such a quest!", GET_NAME(ch));
    } else if (GET_LEVEL(ch) < QST_MINLEVEL(rnum)) {
        snprintf(buf, sizeof(buf), "%s You are not experienced enough for that quest!", GET_NAME(ch));
    } else if (GET_LEVEL(ch) > QST_MAXLEVEL(rnum)) {
        snprintf(buf, sizeof(buf), "%s You are too experienced for that quest!", GET_NAME(ch));
    } else if (is_complete(ch, vnum)) {
        snprintf(buf, sizeof(buf), "%s You have already completed that quest!", GET_NAME(ch));
    } else if ((QST_PREV(rnum) != NOTHING) && !is_complete(ch, QST_PREV(rnum))) {
        snprintf(buf, sizeof(buf), "%s That quest is not available to you yet!", GET_NAME(ch));
        ...
        if (QST_TIME(rnum) != -1) {
            snprintf(buf, sizeof(buf), "%s You have a time limit of %d turn%s to complete the quest.", GET_NAME(ch),
                     QST_TIME(rnum), QST_TIME(rnum) == 1 ? "" : "s");
        } else {
            snprintf(buf, sizeof(buf), "%s You can take however long you want to complete the quest.", GET_NAME(ch));
        }
    }
    do_tell(qm, buf, cmd_tell, 0);
    save_char(ch);
    ...
    for (rnum = 0; rnum < total_quests; rnum++) {
        if (qm == QST_MASTER(rnum)) {
            send_to_char(ch, "\tg%4d\tn) \tc%-52.52s\tn \ty(%5d)\tn \ty(%s)\tn\r\n", ++counter, QST_DESC(rnum),
                         QST_NUM(rnum), (is_complete(ch, QST_NUM(rnum)) ? "Yes" : "No "));
        }
    }
    if (!counter) {
...
static void quest_stat(struct char_data *ch, char argument[MAX_STRING_LENGTH])
{
    ...
    if (!*argument) {
        ...
        switch (QST_TYPE(rnum)) {
            ...
            case AQ_OBJ_RETURN:
                snprintf(targetname, sizeof(targetname), "%s",
                         real_object(QST_TARGET(rnum)) == NOTHING ? "An unknown object" : obj_proto[real_object(
                             QST_TARGET(rnum))].short_description);
                break;
            case AQ_ROOM_FIND:
            case AQ_ROOM_CLEAR:
                snprintf(targetname, sizeof(targetname), "%s",
                         real_room(QST_TARGET(rnum)) == NOWHERE ? "An unknown room" : world[real_room(
                             QST_TARGET(rnum))].name);
                break;
            case AQ_MOB_FIND:
            case AQ_MOB_KILL:
            case AQ_MOB_SAVE:
                snprintf(targetname, sizeof(targetname), "%s",
                         real_mobile(QST_TARGET(rnum)) == NOBODY ? "An unknown mobile" : GET_NAME(
                             &mob_proto[real_mobile(QST_TARGET(rnum))]));
                break;
            default:
        ...
        send_to_char(ch, "VNum  : [\ty%5d\tn], RNum: [\ty%5d\tn] -- Questmaster: [\ty%5d\tn] \ty%s\tn\r\n"
                         "Name  : \ty%s\tn\r\n"
                         "Desc  : \ty%s\tn\r\n"
                         "Accept Message:\r\n\tc%s\tn"
                         "Completion Message:\r\n\tc%s\tn"
                     ...
                     QST_QUANTITY(rnum), QST_POINTS(rnum), QST_PENALTY(rnum), QST_MINLEVEL(rnum), QST_MAXLEVEL(rnum),
                     buf);
        if (QST_PREREQ(rnum) != NOTHING) {
            send_to_char(ch, "Preq  : [\ty%5d\tn] \ty%s\tn\r\n", QST_PREREQ(rnum) == NOTHING ? -1 : QST_PREREQ(rnum),
                         QST_PREREQ(rnum) == NOTHING ? "" : real_object(QST_PREREQ(rnum)) == NOTHING
                                                            ? "an unknown object" : obj_proto[real_object(
                                 QST_PREREQ(rnum))].short_description);
        }
        if (QST_TYPE(rnum) == AQ_OBJ_RETURN) {
            send_to_char(ch, "Mob   : [\ty%5d\tn] \ty%s\tn\r\n", QST_RETURNMOB(rnum),
                         real_mobile(QST_RETURNMOB(rnum)) == NOBODY ? "an unknown mob" : mob_proto[real_mobile(
                             QST_RETURNMOB(rnum))].player.short_descr);
        }
        if (QST_TIME(rnum) != -1) {
