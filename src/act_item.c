    ...
    if (amount <= 0) {
        ...
        if (mode != SCMD_JUNK) {
            ...
            if (mode == SCMD_DONATE) {
                ...
                act("$n throws some gold into the air where it disappears in a puff of smoke!", false, ch, 0, 0,
                    TO_ROOM);
                obj_to_room(obj, RDR);
...
ACMD(do_drop)
{
    ...
    switch (subcmd) {
        ...
        case SCMD_DONATE:
            ...
            num_don_rooms =
                (CONFIG_DON_ROOM_1 != NOWHERE) * 2 + (CONFIG_DON_ROOM_2 != NOWHERE) + (CONFIG_DON_ROOM_3 != NOWHERE) +
                1;
