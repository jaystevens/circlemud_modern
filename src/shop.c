...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
        if (GET_OBJ_COST(obj) > GET_QUESTPOINTS(ch) && !IS_GOD(ch)) {
            ...
            snprintf(actbuf, sizeof(actbuf), "%s You haven't earned enough quest points for such an item.",
                     GET_NAME(ch));
...
list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr, struct char_data *keeper, struct char_data *ch)
{
    ...
    if (shop_producing(obj, shop_nr)) {
        ...
        sprintf(quantity, "%d", cnt);
    }    /* sprintf: OK (for 'quantity >= 11', 32-bit int) */
...
static void shopping_list(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if (!last_obj) {    /* we actually have nothing in our list for sale, period */
        ...
        if (!*name || isname(name, last_obj->name)) {    /* show last obj */
            if (len < sizeof(buf)) {
                strncat(buf, list_object(last_obj, cnt, lindex, shop_nr, keeper, ch), sizeof(buf) - len - 1);
            }
        }    /* strncat: OK */
...
static int read_type_list(FILE *shop_f, struct shop_buy_data *list, int new_format, int max)
{
    ...
    do {
        if (fgets(buf, sizeof(buf), shop_f) == NULL) {
            if (feof(shop_f))
                basic_mud_log("SYSERR: unexpected end of file reading shop file type list.");
            else if (ferror(shop_f))
                basic_mud_log("SYSERR: error reading reading shop file type list: %s", strerror(errno));
            else
                basic_mud_log("SYSERR: error reading reading shop file type list.");
        }
...
void assign_the_shopkeepers(void)
{
    ...
    for (cindex = 0; cindex <= top_shop; cindex++) {
        ...
        if (SHOP_KEEPER(cindex) > top_of_mobt) {
            basic_mud_log("shop %d had mob out of bounds", cindex);
            abort();
        }
...
static void list_all_shops(struct char_data *ch)
{
    const char *list_all_shops_header = " ##   Virtual   Where    Keeper    Buy   Sell   Customers\r\n"
                                        "---------------------------------------------------------\r\n";
