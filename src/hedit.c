...
static void hedit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d, "%s-- Help file editor\r\n"
                       "%s1%s) Entry       :\r\n%s%s"
                       "%s2%s) Min Level   : %s%d\r\n"
                       "%sQ%s) Quit\r\n"
    ...
    switch (OLC_MODE(d)) {
        ...
        case HEDIT_CONFIRM_EDIT:
            switch (*arg) {
                ...
                default:
                    write_to_output(d, "Invalid choice!\r\n"
                                       "Do you wish to edit the '%s' help file? ", help_table[OLC_ZNUM(d)].keywords);
                    break;
            }
            return;
        ...
        case HEDIT_CONFIRM_ADD:
            switch (*arg) {
                ...
                default:
                    write_to_output(d, "Invalid choice!\r\n"
                                       "Do you wish to add the '%s' help file? ", OLC_STORAGE(d));
                    break;
            }
            return;
