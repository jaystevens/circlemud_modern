    ...
    if ((!*arg)) {
        if (GET_LEVEL(ch) >= LVL_GRGOD) {
            send_to_char(ch, "Usage: %s%s submit <header>%s\r\n"
                             ...
                             "       %s%s remove <num>%s\r\n"
                             "       %s%s edit <num>%s\r\n"
                             "       %s%s resolve <num>%s\r\n", QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL,
                         CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM);
            return;
        } else if (GET_LEVEL(ch) >= LVL_IMMORT) {
            send_to_char(ch, "Usage: %s%s submit <header>%s\r\n"
                             "       %s%s list%s\r\n"
                             "       %s%s show <num>%s\r\n", QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME,
                         QNRM);
            return;
        } else {
            send_to_char(ch, "Usage: %s%s submit <header>%s\r\n"
        ...
        if (first_ibt) {
            if (GET_LEVEL(ch) < LVL_IMMORT) {
                len = snprintf(buf, sizeof(buf), "%s No %s|%s Description\r\n"
                                                 "%s ---|-------------------------------------------------%s\r\n", QCYN,
                               QGRN, QCYN, QGRN, QNRM);
            } else {
                len = snprintf(buf, sizeof(buf), "%s No %s|%sName        %s|%sRoom  %s|%sLevel%s|%s Description\r\n"
                                                 "%s ---|------------|------|-----|-------------------------------------------------%s\r\n",
            ...
            if ((num_res + num_unres) > 0) {
                len += snprintf(buf + len, sizeof(buf) - len,
                                ...
                                "%s%ss in %sGREEN%s are resolved %ss.\r\n", QCYN, i, CMD_NAME, QBGRN, num_res, QCYN,
                                QBRED, num_unres, QCYN, QNRM, QCYN, ibt_types[subcmd], QRED, QCYN, CMD_NAME, QCYN,
                                ibt_types[subcmd], QYEL, QCYN, CMD_NAME, QCYN, ibt_types[subcmd], QGRN, QCYN, CMD_NAME);
            } else {
...
static void ibtedit_disp_main_menu(struct descriptor_data *d)
{
    ...
    send_to_char(ch, "%s-- Edit %s Number %s[%s%d%s]\r\n"
                     ...
                     "%s5%s) Details:\r\n%s%s\r\n"
                     "%s6%s) Admin Notes:\r\n%s%s\r\n"
                     "%sQ%s) Quit %s Editor\r\n", QBGRN, IBT_TYPE, cyn, yel, OLC_NUM(d), cyn, yel, nrm, yel,
                 OLC_IBT(d)->name, yel, nrm, cyn, yel, OLC_IBT(d)->room, cyn, nrm,
                 (rr == NOWHERE) ? CCRED(d->character, C_NRM) : cyn,
