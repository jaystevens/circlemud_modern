...
static void oedit_disp_container_flags_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d, "%s1%s) CLOSEABLE\r\n"
                       "%s2%s) PICKPROOF\r\n"
                       "%s3%s) CLOSED\r\n"
                       "%s4%s) LOCKED\r\n"
...
static void oedit_disp_extradesc_menu(struct descriptor_data *d)
{
    ...
    get_char_colors(d->character);
    clear_screen(d);
    write_to_output(d, "Extra desc menu\r\n"
                       "%s1%s) Keywords: %s%s\r\n"
                       "%s2%s) Description:\r\n%s%s\r\n"
...
static void oedit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d, "-- Item number : [%s%d%s]\r\n"
                       "%s1%s) Keywords : %s%s\r\n"
                       "%s2%s) S-Desc   : %s%s\r\n"
                       "%s3%s) L-Desc   :-\r\n%s%s\r\n"
                       "%s4%s) A-Desc   :-\r\n%s%s"
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
    ...
    switch (OLC_MODE(d)) {
        ...
        case OEDIT_TYPE:
            ...
            GET_OBJ_VAL(OLC_OBJ(d), 0) = GET_OBJ_VAL(OLC_OBJ(d), 1) = GET_OBJ_VAL(OLC_OBJ(d), 2) = GET_OBJ_VAL(
                OLC_OBJ(d), 3) = 0;
            break;
