...
static void load_zones(FILE *fl, char *zonename)
{
   ...
   * updated to suit. - ae. */
    ...
    if (sscanf(buf, " %hd %hd %d %d %s %s %s %s %d %d", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode, zbuf1, zbuf2, zbuf3,
               zbuf4, &Z.min_level, &Z.max_level) != 10) {
