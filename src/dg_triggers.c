    ...
    for (ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room) {
        ...
        for (t = TRIGGERS(SCRIPT(ch)); t; t = next_trig) {
            ...
            if (((IS_SET(GET_TRIG_TYPE(t), MTRIG_GREET) && CAN_SEE(ch, actor)) ||
                 IS_SET(GET_TRIG_TYPE(t), MTRIG_GREET_ALL)) && !GET_TRIG_DEPTH(t) &&
                (rand_number(1, 100) <= GET_TRIG_NARG(t))) {
...
void
act_mtrigger(const char_data *ch, char *str, char_data *actor, char_data *victim, obj_data *object, obj_data *target,
             char *arg)
{
    trig_data *t;
    char buf[MAX_INPUT_LENGTH];
