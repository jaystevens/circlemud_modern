...
static void msgedit_main_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d, "   %sDeath Messages:\r\n"
                       "%sA%s) CHAR : %s %s\r\n"
                       "%sB%s) VICT : %s %s\r\n"
                       "%sC%s) ROOM : %s %s\r\n", cyn, grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->die_msg.attacker_msg), grn,
                    yel, nrm, PRINT_MSG(OLC_MSG(d)->die_msg.victim_msg), grn, yel, nrm,
                    PRINT_MSG(OLC_MSG(d)->die_msg.room_msg));

    write_to_output(d, "   %sMiss Messages:\r\n"
                       "%sD%s) CHAR : %s %s\r\n"
                       "%sE%s) VICT : %s %s\r\n"
                       "%sF%s) ROOM : %s %s\r\n", cyn, grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->miss_msg.attacker_msg), grn,
                    yel, nrm, PRINT_MSG(OLC_MSG(d)->miss_msg.victim_msg), grn, yel, nrm,
                    PRINT_MSG(OLC_MSG(d)->miss_msg.room_msg));

    write_to_output(d, "   %sHit Messages:\r\n"
                       "%sG%s) CHAR : %s %s\r\n"
                       "%sH%s) VICT : %s %s\r\n"
                       "%sI%s) ROOM : %s %s\r\n", cyn, grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->hit_msg.attacker_msg), grn,
                    yel, nrm, PRINT_MSG(OLC_MSG(d)->hit_msg.victim_msg), grn, yel, nrm,
                    PRINT_MSG(OLC_MSG(d)->hit_msg.room_msg));

    write_to_output(d, "   %sGod Messages:\r\n"
                       "%sJ%s) CHAR : %s %s\r\n"
                       "%sK%s) VICT : %s %s\r\n"
                       "%sL%s) ROOM : %s %s\r\n", cyn, grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->god_msg.attacker_msg), grn,
                    yel, nrm, PRINT_MSG(OLC_MSG(d)->god_msg.victim_msg), grn, yel, nrm,
