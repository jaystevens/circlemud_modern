            ...
            switch (GET_OBJ_TYPE(obj)) {
                case ITEM_NOTE:
                    if (obj->action_description) {
                        ...
                        page_string(ch->desc, notebuf, false);
                    } else {
...
static void list_one_char(struct char_data *i, struct char_data *ch)
{
    ...
    if (GROUP(i)) {
        if (GROUP(i) == GROUP(ch)) {
            send_to_char(ch, "(%s%s%s) ", CBGRN(ch, C_NRM), GROUP_LEADER(GROUP(i)) == i ? "leader" : "group",
                         CCNRM(ch, C_NRM));
        } else {
            send_to_char(ch, "(%s%s%s) ", CBRED(ch, C_NRM), GROUP_LEADER(GROUP(i)) == i ? "leader" : "group",
                         CCNRM(ch, C_NRM));
        }
    }
    ...
    if (GET_POS(i) != POS_FIGHTING) {
        if (!SITTING(i)) {
            ...
            send_to_char(ch, " is %s upon %s.",
                         (GET_POS(i) == POS_SLEEPING ? "sleeping" : (GET_POS(i) == POS_RESTING ? "resting"
                                                                                               : "sitting")),
                         OBJS(furniture, ch));
        }
    } else {
    ...
    for (door = 0; door < DIR_COUNT; door++) {
        ...
        if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWVNUMS) && !EXIT_FLAGGED(EXIT(ch, door), EX_CLOSED)) {
            ...
            send_to_char(ch, "%-5s - %s\r\n", dirs[door],
                         IS_DARK(EXIT(ch, door)->to_room) && !CAN_SEE_IN_DARK(ch) ? "Too dark to tell." : world[EXIT(ch,
                                                                                                                     door)->to_room].name);
        }
    }
...
static void look_at_target(struct char_data *ch, char *arg)
{
    ...
    bits = generic_find(arg, FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP | FIND_CHAR_ROOM, ch, &found_char,
                        &found_obj);
...
ACMD(do_score)
{
    ...
    send_to_char(ch, "You have completed %d quest%s, ", GET_NUM_QUESTS(ch), GET_NUM_QUESTS(ch) == 1 ? "" : "s");
    ...
    if (OUTSIDE(ch)) {
        ...
        if (GET_LEVEL(ch) >= LVL_GOD) {
            send_to_char(ch, "Pressure: %d (change: %d), Sky: %d (%s)\r\n", weather_info.pressure, weather_info.change,
                         weather_info.sky, sky_look[weather_info.sky]);
        }
    } else {
...
ACMD(do_users)
{
    ...
    send_to_char(ch, "Num Class   Name         State          Idl   Login\t*   Site\r\n"
                     "--- ------- ------------ -------------- ----- -------- ------------------------\r\n");
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        sprintf(line, "%3d %-7s %-12s %-14s %-3s %-8s ", d->desc_num, classname,
                d->original && d->original->player.name ? d->original->player.name : d->character &&
                                                                                     d->character->player.name
                                                                                     ? d->character->player.name
                                                                                     : "UNDEFINED", state, idletime,
                timestr);
    ...
    if (!*arg) {
        ...
        for (i = character_list; i; i = i->next) {
            if (CAN_SEE(ch, i) && IN_ROOM(i) != NOWHERE && isname(arg, i->player.name)) {
                found = 1;
                send_to_char(ch, "M%3d. %-25s%s - [%5d] %-25s%s", ++num, GET_NAME(i), QNRM, GET_ROOM_VNUM(IN_ROOM(i)),
...
ACMD(do_toggle)
{
    ...
    if (!*arg) {
        ...
        if (GET_LEVEL(ch) == LVL_IMPL) {
            send_to_char(ch, " SlowNameserver: %-3s   "
                             "                        "
                             " Trackthru Doors: %-3s\r\n",
        ...
        if (GET_LEVEL(ch) >= LVL_IMPL) {
            send_to_char(ch, "     ZoneResets: %-3s\r\n", ONOFF(PRF_FLAGGED(ch, PRF_ZONERESETS)));
        }

        send_to_char(ch, "Hit Pnt Display: %-3s    "
                         "          Brief: %-3s    "
                         "     Summonable: %-3s\r\n"
                     ...
                     ONOFF(PRF_FLAGGED(ch, PRF_AUTOKEY)), ONOFF(PRF_FLAGGED(ch, PRF_AUTODOOR)), types[COLOR_LEV(ch)]);
        return;
    }
...
ACMD(do_whois)
{
    ...
    send_to_char(ch, "Name: %s %s\r\nSex: %s\r\n", GET_NAME(victim), (victim->player.title ? victim->player.title : ""),
                 buf);
