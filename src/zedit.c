...
static void zedit_disp_menu(struct descriptor_data *d)
{
    ...
    /* Menu header */
    send_to_char(d->character, "Room number: %s%d%s Room zone: %s%d\r\n"
                               "%s1%s) Builders       : %s%s\r\n"
                               "%sZ%s) Zone name      : %s%s\r\n"
                 ...
                 OLC_ZONE(d)->reset_mode ? ((OLC_ZONE(d)->reset_mode == 1) ? "Reset when no players are in zone."
                                                                           : "Normal reset.") : "Never reset", grn, nrm,
                 cyn, buf1, grn, nrm, levels_set ? cyn : yel, lev_string, nrm);
    ...
    while (MYCMD.command != 'S') {
        ...
        switch (MYCMD.command) {
            ...
            case 'D':
                ...
                break;
            case 'T':
                write_to_output(d, "%sAttach trigger %s%s%s [%s%d%s] to %s", MYCMD.if_flag ? " then " : "", cyn,
                                trig_index[MYCMD.arg2]->proto->name, yel, cyn, trig_index[MYCMD.arg2]->vnum, yel,
                                ((MYCMD.arg1 == MOB_TRIGGER) ? "mobile" : ((MYCMD.arg1 == OBJ_TRIGGER) ? "object"
                                                                                                       : ((MYCMD.arg1 ==
                                                                                                           WLD_TRIGGER)
                                                                                                          ? "room"
                                                                                                          : "????"))));
                break;
            case 'V':
                write_to_output(d, "%sAssign global %s:%d to %s = %s", MYCMD.if_flag ? " then " : "", MYCMD.sarg1,
                                MYCMD.arg2,
                                ((MYCMD.arg1 == MOB_TRIGGER) ? "mobile" : ((MYCMD.arg1 == OBJ_TRIGGER) ? "object"
                                                                                                       : ((MYCMD.arg1 ==
                                                                                                           WLD_TRIGGER)
    ...
    /* Finish off menu */
    write_to_output(d, "%s%d - <END OF LIST>\r\n"
...
static void zedit_disp_comtype(struct descriptor_data *d)
{
    ...
    write_to_output(d, "\r\n"
                       ...
                       "\r\n"
                       "What sort of command will this be? : ", grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn,
